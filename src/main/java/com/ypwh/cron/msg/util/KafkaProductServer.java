...
import com.ypwh.cron.chuda.service.impl.ChuDaServiceImpl;
 ...
 *
 */
@Component("kafkaProductServer")
public class KafkaProductServer{
	private static final Logger logger = LoggerFactory.getLogger(KafkaProductServer.class);
    @Autowired
     ...
     * kafka发送消息模板
     * @param topic 主题
     * @param valueString    messageValue json格式
     * @param ifPartition 是否使用分区 0是\1不是
     * @param partitionNum 分区数 如果是否使用分区为0,分区数必须大于0
     */
    public Map<String,Object> sndMesForTemplate(String topic,String key, String valueString, String ifPartition, 
            Integer partitionNum){
        if(ifPartition.equals("0")){
            //表示使用分区
            int partitionIndex = getPartitionIndex(key, 3);//三个分区 先写死
